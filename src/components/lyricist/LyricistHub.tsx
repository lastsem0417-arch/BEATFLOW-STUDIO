    ...
    <div className="h-[75vh] min-h-[600px] w-full bg-[#0A0A0C] border border-white/5 rounded-[2rem] flex overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative select-none mt-2">
      ...
      <div className="w-80 shrink-0 border-r border-white/5 bg-[#030305]/90 flex flex-col z-10 backdrop-blur-2xl">
        <div className="p-8 border-b border-white/5 bg-[#010101]">
            <h2 className="text-3xl font-serif italic text-[#F0F0EB]">Network<span className="text-[#52B788]">.</span></h2>
        ...
        <div className="p-5 border-b border-white/5 bg-[#010101]">
            <div className="relative flex items-center">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-2.5 text-xs text-[#F0F0EB] font-mono outline-none focus:border-[#52B788]/50 transition-all placeholder:text-[#888888]/50" 
              />
            </div>
        </div>
            ...
            {filteredUsers.map((u: any) => {
              ...
              return (
                <div 
                  ...
                  className={`group flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all duration-300 ${isActive ? 'bg-[#52B788]/10 border border-[#52B788]/30 shadow-[0_0_20px_rgba(82,183,136,0.1)]' : 'hover:bg-white/[0.03] border border-transparent hover:border-white/5'}`}
                >
                  <div className={`w-12 h-12 rounded-full border bg-[#010101] overflow-hidden transition-all ${isActive ? 'border-[#52B788] shadow-[0_0_10px_rgba(82,183,136,0.4)]' : 'border-white/10 group-hover:border-white/30'}`}>
        ...
        {selectedUser ? (
          <div className="flex-1 flex overflow-hidden relative z-10 h-full">
                ...
                <div className="w-80 shrink-0 border-r border-white/5 bg-[#030305]/50 p-8 flex flex-col h-full">
                    <div className="text-center mb-8 border-b border-white/5 pb-8 relative group">
                        <div className="absolute inset-0 bg-[#52B788]/5 blur-[40px] rounded-full pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
                    ...
                    <p className="text-[9px] uppercase tracking-[0.4em] text-[#52B788] font-black mb-5">Public Assets</p>
                    ...
                    <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1" data-lenis-prevent="true">
                        {userPortfolio.length > 0 ? userPortfolio.map(track => {
                          ...
                          return (
                            <div key={track._id} className={`flex justify-between p-4 rounded-2xl items-center border transition-all duration-300 ${isPlaying ? 'bg-[#52B788]/5 border-[#52B788]/30' : 'bg-[#010101] border-white/5 hover:border-[#52B788]/20'}`}>
                                <div className="overflow-hidden pr-3">
                                    <h4 className={`text-xs truncate w-32 font-bold transition-colors ${isPlaying ? 'text-[#52B788]' : 'text-[#F0F0EB]'}`}>{track.title}</h4>
                                    <p className="text-[8px] uppercase tracking-widest text-[#888888] mt-1 font-mono">{track.trackType}</p>
                                </div>
                                <button 
                                  onClick={() => togglePlay(track._id)} 
                                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isPlaying ? 'bg-[#52B788] text-black shadow-[0_0_15px_rgba(82,183,136,0.4)]' : 'bg-white/5 text-[#F0F0EB] border border-white/10 hover:border-[#52B788] hover:text-[#52B788]'}`}
                                >
                                    {isPlaying ? (
                ...
                <div className="flex-1 flex flex-col relative bg-[#010101] h-full">
                    <div className="h-20 border-b border-white/5 bg-[#030305]/80 flex justify-between items-center px-8 z-20 backdrop-blur-3xl shrink-0">
                        <div className="flex items-center gap-4">
                            <div className="w-2 h-2 bg-[#52B788] rounded-full animate-pulse shadow-[0_0_10px_#52B788]"></div>
                            <div>
                          ...
                          chatMessages.map((msg: any, i: number) => {
                            ...
                            return (
                              <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[75%] p-4 rounded-[1.5rem] flex flex-col gap-2 shadow-lg backdrop-blur-md transition-all ${isMe ? 'bg-[#52B788]/10 border border-[#52B788]/30 text-[#F0F0EB] rounded-tr-sm' : 'bg-white/[0.03] border border-white/10 text-[#F0F0EB] rounded-tl-sm'}`}>
                                  <p className="text-[13px] font-light leading-relaxed tracking-wide whitespace-pre-wrap">{msg.text}</p>
                                  <p className={`text-[8px] uppercase tracking-widest font-mono text-right ${isMe ? 'text-[#52B788]/70' : 'text-[#888888]'}`}>
                                    {msg.timestamp}
                                  </p>
                                </div>
                              </div>
                    ...
                    <div className="p-6 bg-[#030305] border-t border-white/5 flex flex-col gap-4 backdrop-blur-3xl relative z-20 shrink-0">
                       <div className="flex justify-start">
                           <button 
                                onClick={triggerAIGhostwriter} 
                                disabled={isAiLoading}
                                className="flex items-center gap-2 px-4 py-2 border border-[#52B788]/50 text-[#52B788] rounded-full text-[9px] uppercase tracking-widest font-black hover:bg-[#52B788] hover:text-[#010101] hover:shadow-[0_0_15px_rgba(82,183,136,0.3)] transition-all disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-[#52B788]"
                           >
                       ...
                       <div className="flex gap-4 items-end">
                           <textarea 
                             ...
                             className="flex-1 bg-white/5 rounded-2xl p-4 text-sm text-[#F0F0EB] font-mono outline-none focus:border-[#52B788] focus:bg-white/10 border border-transparent transition-all resize-none custom-scrollbar min-h-[50px] max-h-32 placeholder:text-[#888888]/50" 
                             rows={1}
                           />
                           <button 
                             onClick={handleSendMessage} 
                             className="w-14 h-14 bg-[#F0F0EB] border border-transparent hover:border-[#52B788] rounded-full text-black hover:text-[#52B788] hover:bg-black hover:scale-105 active:scale-95 shadow-lg hover:shadow-[0_0_20px_rgba(82,183,136,0.4)] transition-all flex items-center justify-center shrink-0 mb-1"
                           >
          ...
          <div className="flex-1 flex flex-col items-center justify-center h-full opacity-40 select-none bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] mix-blend-screen">
             <div className="w-32 h-32 rounded-full border border-white/10 flex items-center justify-center mb-8 relative">
                <div className="absolute inset-0 rounded-full border border-[#52B788]/30 animate-[ping_3s_infinite]"></div>
             ...
             <h2 className="text-4xl font-serif italic text-[#F0F0EB] tracking-tight">Select a Collaborator</h2>
